    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            if (isset($pathinfo['dirname'])) {
                if (!$this->getContainer()->get("TTFileUtils")->fileExists($pathinfo['dirname'])) {
                    $dir_created = mkdir($pathinfo['dirname'], 0777, true);
                }
            }

            if ($this->getContainer()->get("TTFileUtils")->fileExists($filename)) {
                $this->getContainer()->get("TTFileUtils")->unlinkFile($filename);
            }
