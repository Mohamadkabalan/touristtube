...
Marquee.prototype = {
	...
	add: function(element) {
		...
		if (options.coords.width || options.coords.x2){
                    this.setCoords( options.coords.x1, options.coords.y1, options.coords.width ? options.coords.width : options.coords.x2 - options.coords.x1, options.coords.height ? options.coords.height : options.coords.y2 - options.coords.y1 );
                }
	...
	updateDrag: function(event) {
		...
		this.setCoords( this.startOffset[0] + shifting[0], this.startOffset[1] + shifting[1], newDimension.width, newDimension.height );
	...
	setCoords: function(x, y, w, h) {
		...
		var maxWidth = this.items[this.activeId].options.coords.width;
		var maxHeight = this.items[this.activeId].options.coords.height;
		w = Math.abs(w);
		h = Math.abs(h);
		var mAx_X = Math.abs(1024 - w);
		var mAx_Y = Math.abs(600 - h);
