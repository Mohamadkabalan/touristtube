(function(factory)
	...
	}(function($)
    {
        ...
        function TagSuggest(element, options)
        {
	        var ms = this;
	        ...
	        var defaults = {
	            ...
	            /**
				 * @cfg {Array / String / Function} data JSON Data source used to populate the combo box. 3 options
				 *      are available here:<br/>
				 *      <p>
				 *      <u>No Data Source (default)</u><br/> When left null, the combo box will not suggest
				 *      anything. It can still enable the user to enter multiple entries if allowFreeEntries is *
				 *      set to true (default).
				 *      </p>
				 *      <p>
				 *      <u>Static Source</u><br/> You can pass an array of JSON objects, an array of strings or
				 *      even a single CSV string as the data source.<br/>For ex. data: [* {id:0,name:"Paris"}, {id:
				 *      1, name: "New York"}]<br/> You can also pass any json object with the results property
				 *      containing the json array.
				 *      </p>
				 *      <p>
				 *      <u>Url</u><br/> You can pass the url from which the component will fetch its JSON data.<br/>Data
				 *      will be fetched using a POST ajax request that will * include the entered text as 'query'
				 *      parameter. The results fetched from the server can be: <br/> - an array of JSON objects (ex:
				 *      [{id:...,name:...},{...}])<br/> - a string containing an array of JSON objects ready to be
				 *      parsed (ex: "[{id:...,name:...},{...}]")<br/> - a JSON object whose data will be contained
				 *      in the results property (ex: {results: [{id:...,name:...},{...}]
				 *      </p>
				 *      <p>
				 *      <u>Function</u><br/> You can pass a function which returns an array of JSON objects (ex:
				 *      [{id:...,name:...},{...}])<br/> The function can return the JSON data or it can use the
				 *      first argument as function to handle the data.<br/> Only one (callback function or return
				 *      value) is needed for the function to succeed.<br/> See the following example:<br/>
				 ...
				 *      <p>
				 *      Set to false if you only want mouse interaction. In that case the combo will automatically
				 *      expand on focus.
				 *      </p>
				 *      Defaults to <code>true</code>.
				 */
				 ...
				 *      </p>
				 *      Defaults to <code>'Type or click here'</code> or just <code>'Click here'</code> if not
				 *      editable.
				 */
	            emptyText : function()
	            {
				 ...
				 * @cfg {Object} inputCfg
				 *      <p>
				 *      Additional parameters passed out to the INPUT tag. Enables usage of AngularJS's custom tags
				 *      for ex.
				 *      </p>
				 *      Defaults to <code>{}</code>
				 */
				 ...
				 *      <p>
				 *      Set to true to filter data results according to case. Useless if the data is fetched
				 *      remotely
				 *      </p>
				 *      Defaults to <code>false</code>.
				 */
				 ...
				 *      <p>
				 *      Once expanded, the combo's height will take as much room as the # of available results. In
				 *      case there are too many results displayed, this will fix the drop down height.
				 *      </p>
				 *      Defaults to 290 px.
				 */
	            maxDropHeight : 290,
				 ...
				 *      <p>
				 *      The maximum number of items the user can select if multiple selection is allowed. Set to
				 ...
				 *      <p>
				 *      A function that defines the helper text when the max selection amount has been reached. The
				 *      function has a single parameter which is the number of selected elements.
				 *      </p>
				 *      Defaults to
				 ...
				 *      <p>
				 *      The minimum number of characters the user must type before the combo expands and offers
				 *      suggestions. Defaults to <code>0</code>.
				 */
	            minChars : 0,
				 ...
				 *      <p>
				 *      A function that defines the helper text when not enough letters are set. The function has a
				 *      single parameter which is the difference between the required amount of letters and the
				 *      current one.
				 *      </p>
				 *      Defaults to
				 ...
				 *      <p>
				 *      Where the selected items will be displayed. Only 'right', 'bottom' and 'inner' are valid
				 *      values
				 *      </p>
				 *      Defaults to <code>'inner'</code>, meaning the selected items will appear within the input
				 *      box itself.
				 */
				 ...
				 *      <p>
				 *      name of JSON object property for local result sorting. Leave null if you do not wish the
				 *      results to be ordered or if they are already ordered remotely.
				 *      </p>
				 * 
				 * Defaults to <code>null</code>.
				 */
	        ...
	        {
		        this.removeFromSelection(_selection.slice(0), isSilent); // clone
		        // array
		        // to
		        // avoid
		        // concurrency
	        ...
	        this.expand =
	                ...
	                {
		                if (!cfg.expanded
		                        && (this.input.val().length >= cfg.minChars || this.combobox.children().size() > 0)) {
	        ...
	        /**
			 * Sets a value for the combo box. Value must be a value or an array of value with data type matching
			 * valueField one.
			 * 
			 * @param data
			 */
	        ...
	        _timer, _hasFocus = false, _groups = null, _cbData = [], _ctrlDown = false;
	        ...
	        var self =
	                {
	                    ...
	                    _displaySuggestions : function(data)
	                    {
		                    ...
		                    var resHeight = 0, // total height taken by
		                    // displayed results.
		                    nbGroups = 0;
	                    ...
	                    _processSuggestions : function(source)
	                    {
		                    ...
		                    if (data !== null) {
			                    ...
			                    if (typeof (data) === 'string' && data.indexOf(',') < 0) { // get
				                    // results
				                    // from
				                    // ajax
				                    $(ms).trigger('beforeload', [ ms ]);
				                    cfg.queryName = cfg.queryName || "q";
				                    var params = {};
				                    	params[cfg.queryName] = ms.input.val().trim();
				                    ...
				                    $.ajax({
				                        ...
				                        {
					                        json =
					                                typeof (asyncData) === 'string' ? JSON.parse(asyncData)
					                                        : asyncData;
				                    ...
				                    return;
			                    } else if (typeof (data) === 'string' && data.indexOf(',') > -1) { // results
				                    // from
				                    // csv
				                    // string
				                    _cbData = self._getEntriesFromStringArray(data.split(','));
			                    } else { // results from local array
				                    if (data.length > 0 && typeof (data[0]) === 'string') { // results
					                    // from
					                    // array
					                    // of
					                    // strings
					                    _cbData = self._getEntriesFromStringArray(data);
				                    } else { // regular json array or
	                    ...
	                    _render : function(el)
	                    {
		                    ...
		                    // holds the main div, will relay the focus
		                    // events to the contained input element.
		                    ms.container =
		                            $(
		                                    '<div/>',
		                                    {
		                    ...
		                    // holds the suggestions. will always be placed
		                    // on focus
		                    ms.combobox = $('<div/>', {
		                    ...
		                    // bind the onclick and mouseover using
		                    // delegated events (needs jQuery >= 1.7)
		                    ms.combobox.on('click', 'div.tag-res-item', $
		                            .proxy(handlers._onComboItemSelected, this));
		                    ms.combobox.on('mouseover', 'div.tag-res-item', $.proxy(handlers._onComboItemMouseOver,
		                            this));
	                    ...
	                    {
		                    if (cfg.onRenderComboItems) {
			                    items = cfg.onRenderComboItems(items, isGrouped);
		                    }
		                    //
		                    var ref = this, html = '';
		                    ...
		                    {
			                    var displayed =
			                            cfg.renderer !== null ? cfg.renderer.call(ref, value)
			                                    : value[cfg.displayField];
			                    var resultItemEl =
			                            $('<div/>', {
			                                'class' : 'tag-res-item '
			                                        + (isGrouped ? 'tag-res-item-grouped ' : '')
			                                        + (index % 2 === 1 && cfg.useZebraStyle === true
			                                                ? 'tag-res-odd' : ''),
			                                html : cfg.highlight === true ? self._highlightSuggestion(displayed)
			                                        : displayed,
	                    ...
	                    {
		                    var ref = this, w = 0, inputOffset = 0, items = [], asText =
		                            cfg.resultAsString === true && !_hasFocus;
		                    ...
		                    $.each(_selection, function(index, value)
		                    {
			                    ...
			                    var selectedItemEl, delItemEl, selectedItemHtml =
			                            cfg.selectionRenderer !== null ? cfg.selectionRenderer.call(ref, value)
			                                    : value[cfg.displayField];
	                    ...
	                    _selectItem : function(item)
	                    {
		                    ...
		                    if (cfg.onSelectItem && cfg.onSelectItem(item.data('json'))) {
			                    ms.collapse();
		                    } else {
	                    ...
	                    {
		                    var q = ms.getRawValue(), filtered = [], newSuggestions = [], selectedValues =
		                            ms.getValue();
		                    ...
		                    if (q.length > 0) {
			                    $.each(data, function(index, obj)
			                    {
				                    ...
				                    if ((cfg.matchCase === true && name.indexOf(q) > -1)
				                            || (cfg.matchCase === false && name.toLowerCase().indexOf(
				                                    q.toLowerCase()) > -1)) {
		                    ...
		                    // take out the ones that have already been
		                    // selected
	        ...
	        var handlers =
	                {
	                    ...
	                    /**
						 * Triggered when the user presses a key while the component has focus This is where we want
						 * to handle all keys that don't require the user input field since it hasn't registered the
						 * key hit yet
						 * 
						 * @param e
						 *            keyEvent
	                    ...
	                    _onKeyDown : function(e)
	                    {
		                    ...
		                    var active = ms.combobox.find('.tag-res-item-active:first'), freeInput =
		                            ms.input.val() !== cfg.emptyText ? ms.input.val() : '';
		                    $(ms).trigger('keydown', [ ms, e ]);

		                    if (e.keyCode === 9
		                            && (cfg.useTabKey === false || (cfg.useTabKey === true && active.length === 0 && ms.input
		                                    .val().length === 0))) {
			                    handlers._onBlur();
			                    return;
		                    }
		                    ...
		                    {
			                    case 8: // backspace
				                    if (freeInput.length === 0 && ms.getSelectedItems().length > 0
	                    ...
	                    _onKeyUp : function(e)
	                    {
		                    var freeInput = ms.getRawValue(), inputValid =
		                    ...
		                    switch (e.keyCode)
		                    {
			                    ...
			                    case 188:// enter, tab, comma
				                    if (e.keyCode !== 188 || cfg.useCommaKey === true) {
					                    ...
					                    // if no selection or if freetext
					                    // entered and free entries allowed,
					                    // add new obj to selection
					                    if (inputValid === true && cfg.allowFreeEntries === true) {
						                    obj[cfg.displayField] = obj[cfg.valueField] = freeInput;
						                    ms.addToSelection(obj);
						                    ms.collapse(); // reset combo
						                    // suggestions
						                    ms.input.focus();
					                    }
					                    break;
			                    ...
			                    default:
				                    if (_selection.length === cfg.maxSelection) {
					                    ...
					                    if (freeInput.length < cfg.minChars) {
						                    ...
						                    self._updateHelper(cfg.maxEntryRenderer.call(this, freeInput.length
						                            - cfg.maxEntryLength));
						                    if (cfg.expanded === true) {
							                    ms.collapse();
						                    }
		                    ...
		                    if (ms.isDisabled() === false
			                    ...
			                    if (cfg.expanded === true) {
				                    ...
				                    if (curLength >= cfg.minChars) {
					                    ...
					                    self._updateHelper(cfg.minCharsRenderer
					                            .call(this, cfg.minChars - curLength));
				                    }
			                    }
		                    }
	                    }
        ...
        $.fn.tagSuggest = function(options)
        {
	        ...
	        obj.each(function(i)
	        {
		        ...
		        if (this.nodeName.toLowerCase() === 'select') { // rendering
			        // from
			        // select
			        options.data = [];
			        options.value = [];
