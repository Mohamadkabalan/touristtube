$( document ).ready(function(){
    ...
    $(document).on('click', ".media_a", function (e) {
        e.preventDefault();
    ...
    $(document).on('click',".discover_a, .prev_pg, .next_pg, .first_pg, .last_pg" ,function(){
        ...
        var data_cid = $('.globcontainer').attr('data-cid');
        var data_state = $('.globcontainer').attr('data-state');
        var data_country = $('.globcontainer').attr('data-country');
        var data_rc = $('.globcontainer').attr('data-rc');
        var data_type = $rowsect.find('.hotelin').attr('data-type');
        
        $.ajax({
            url: generateLangURL('/ajax/dicoverNewData'),
            data: { page:data_page,type:data_type, data_cid:data_cid, data_state:data_state, data_country:data_country, data_rc:data_rc },
            type: 'post',
            success: function(res){
                $rowsect.find('.pagerWrapper').html(res.paging);
                $rowsect.find('.hotelin').html(res.data);                
                initAddToBag();
                $('html, body').animate({
                    scrollTop: $rowsect.find('.hotelin .sect-desp:last-child').last().offset().top
                }, 500);
            }
        });
    });
