		...
		TTCallAPI({
			...
			callback: function(ret){
				...
				if(ret.status == 'ok'){
					...
					$('#like').attr('src', ReturnLink('/media/images/like_button_yellow.png') );
					$('#dislike').attr('src', ReturnLink('/media/images/dislike_button.png') );
				}else{
					TTAlert({
						msg: ret.error_msg,
						type: 'alert',
		...
		TTCallAPI({
			...
			callback: function(ret){
				...
				if(ret.status == 'ok'){
					var cur_likes = parseInt( $('#display_likes').text() );
					$('#display_likes').text( cur_likes - 1);
					$('#dislike').attr('src', ReturnLink('/media/images/dislike_button_yellow.png') );
					$('#like').attr('src', ReturnLink('/media/images/like_button.png') );
				}else{
					TTAlert({
						msg: ret.error_msg,
						type: 'alert',
