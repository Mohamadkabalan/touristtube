...
foreach($langarray as $lang_val)
{
    ...
    //if($lang_val=='en') {
        // sitemap_hotel.xml
        echo PHP_EOL.strftime('%Y-%m-%d %T').' '.$path .$prefixlang. "sitemap_hotel.xml".PHP_EOL;
        $sitemapindex_handle = fopen($path .$prefixlang. "sitemap_hotel.xml", "w");	
        ...
        while($pageloop) {
            ...
            $end = 25000;
            $start = $page * $end;
            $query = "SELECT id, name FROM cms_hotel where published = 1 ORDER BY id ASC LIMIT $start, $end";
            ...
            if(!$res || !$ret) {
                ...
                $sitemap_loop_name = "sitemap/".$prefixlang."sitemap_hotel$pagename.xml";
                ...
                foreach($rows as $indx => $row_item) {
                    ...
                    if($lang_val != 'en') fprintf($sitemap_loop_handle, "<url><loc>https://www.touristtube.com/$lang_val/hotel-details-%s-%s</loc><changefreq>%s</changefreq></url>\n",$title ,$id, $changeFreq);
                    else fprintf($sitemap_loop_handle, "<url><loc>https://www.touristtube.com/hotel-details-%s-%s</loc><changefreq>%s</changefreq></url>\n",$title ,$id, $changeFreq);
