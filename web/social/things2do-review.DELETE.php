...
$includes = array('media'=>'css_media_query/media_style_static_page.css?v='.MQ_MEDIA_STYLE_CSS_V,'media1'=>'css_media_query/hotel-review.css?v='.MQ_HOTEL_REVIEW_CSS_V,'css/social-actions.css?v='.SOCIAL_ACTIONS_CSS_V, 'css/hotel-resto.css', 'css/hotel-review.css',
...
if (userIsLogged()) {
    ...
    $channelrelatedlist = getDiscoverToChannelid($txt_id,SOCIAL_ENTITY_LANDMARK);
...
$media_hotels = getPOIDefaultPic($txt_id);
if( $media_hotels ){
    ...
    $data['discover_img'] = ReturnLink('images/landmark-icon.jpg');
}
...
$hotel_data = getPoiInfo($txt_id);
if(!$hotel_data) header('Location:' . ReturnLink('/') );
$data['longitudeVal'] = $hotel_data['longitude'];
$data['latitudeVal'] = $hotel_data['latitude'];

$data['mapImageURL'] = mapImageURL($hotel_data['latitude'],$hotel_data['longitude'],SOCIAL_ENTITY_LANDMARK,12,'460x110',$hotel_data);
$data['show_on_map']     =   ReturnLink('parts/show-on-map.php?type=p&id=' . $hotel_data['id']);
...
$page_link = ReturnLink('things2do/id/' . $hotel_data['id']);

$data['discover_title'] = $title;
$data['discover_title1'] = $hotel_data['name'];
$data['discover_nam'] = cleanTitle($hotel_data['name']).'_poi_';
$data['discover_location'] = $locationText;
$data['discover_link'] = $page_link;
$data['data_stars'] = 0;
$nb_votes = 0;
$data['nb_votes'] = $nb_votes;
$irating_average = socialRateAverage($hotel_data['id'], array(SOCIAL_ENTITY_LANDMARK));
...
$data['txt_id'] = $txt_id;
$data['SOCIAL_ENTITY_TYPE'] = SOCIAL_ENTITY_LANDMARK;
$data['evalText'] = _('your evaluation for this point of interest');
$data['nearText'] = _("what's near the point of interest?");
...
$media_hotelsi = array();
if (userIsLogged()) {  
//    $query_hotelsi = "SELECT id , filename as img , poi_id FROM `discover_poi_images` WHERE poi_id = $txt_id AND user_id=$user_id ORDER BY id DESC";
    $query_hotelsi = "SELECT id , filename as img , poi_id FROM `discover_poi_images` WHERE poi_id = :Txt_id AND user_id=:User_id ORDER BY id DESC";
...
$params = array();  
if (userIsLogged()) {
//    $query_hotelsi1 = "SELECT id , filename as img , poi_id FROM `discover_poi_images` WHERE poi_id = $txt_id AND user_id<>$user_id ORDER BY id DESC";
    $query_hotelsi1 = "SELECT id , filename as img , poi_id FROM `discover_poi_images` WHERE poi_id = :Txt_id AND user_id<>:User_id ORDER BY id DESC";
...
//    $query_hotelsi1 = "SELECT id , filename as img , poi_id FROM `discover_poi_images` WHERE poi_id = $txt_id ORDER BY id DESC";
    $query_hotelsi1 = "SELECT id , filename as img , poi_id FROM `discover_poi_images` WHERE poi_id = :Txt_id ORDER BY id DESC";
    $params2[] = array(  "key" => ":Txt_id",
                         "value" =>$txt_id);
}
...
$media_hotels_reviews = userReviewsList($txt_id, SOCIAL_ENTITY_LANDMARK, 6, 0);
$media_hotels_reviews_count = userReviewsList($txt_id, SOCIAL_ENTITY_LANDMARK, 0, 0, true);
...
foreach ($media_hotels_reviews as $rev) {
    ...
    $div_val = '<div id="commentDiv" class="social_data_all" data-category="" data-channel="0" data-enable="1" data-id="' . $rev_id . '" data-type="' . SOCIAL_ENTITY_LANDMARK_REVIEWS . '">';
...
$data['dataType'] = SOCIAL_ENTITY_LANDMARK;
$data['dataId'] = $txt_id;

$toRateArr = array();
$irating1 = socialRated($user_id, $txt_id, SOCIAL_ENTITY_LANDMARK_FOODAVAILABLE);
if(!$irating1) $irating1=0;
$irating2 = socialRated($user_id, $txt_id, SOCIAL_ENTITY_LANDMARK_STAIRS);
if(!$irating2) $irating2=0;
$irating3 = socialRated($user_id, $txt_id, SOCIAL_ENTITY_LANDMARK_WHEELCHAIR);
if(!$irating3) $irating3=0;
$irating4 = socialRated($user_id, $txt_id, SOCIAL_ENTITY_LANDMARK_PARKING);
if(!$irating4) $irating4=0;
$irating5 = socialRated($user_id, $txt_id, SOCIAL_ENTITY_LANDMARK_BATHROOMFACILITIES);
if(!$irating5) $irating5=0;
$irating6 = socialRated($user_id, $txt_id, SOCIAL_ENTITY_LANDMARK_STORAGE);
if(!$irating6) $irating6=0;

$toRateArr[] = array('id'=>SOCIAL_ENTITY_LANDMARK_FOODAVAILABLE,'val'=>$irating1,'name'=>_('Food available'));
$toRateArr[] = array('id'=>SOCIAL_ENTITY_LANDMARK_STAIRS,'val'=>$irating2,'name'=>_('Stairs, elevator'));
$toRateArr[] = array('id'=>SOCIAL_ENTITY_LANDMARK_WHEELCHAIR,'val'=>$irating3,'name'=>_('Wheelchair access'));
$toRateArr[] = array('id'=>SOCIAL_ENTITY_LANDMARK_PARKING,'val'=>$irating4,'name'=>_('Stroller parking'));
$toRateArr[] = array('id'=>SOCIAL_ENTITY_LANDMARK_BATHROOMFACILITIES,'val'=>$irating5,'name'=>_('Bathroom facilities'));
$toRateArr[] = array('id'=>SOCIAL_ENTITY_LANDMARK_STORAGE,'val'=>$irating6,'name'=>_('Lockers, storage'));
