...
$template = $twig->loadTemplate('channel-videos.twig');
...
$channelvideoInfo = mediaSearch(array(
...
$data['channelvideoInfoCount'] = $channelvideoInfoCount;

$data['channelvideoInfo']= $channelvideoInfo;

$data['formatChannelvideoInfoCount'] = tt_number_format($channelvideoInfoCount);
...
$channelvideoInfoArr = array();
$i = 0;
foreach ($channelvideoInfo as $VidInfo) {
    $i++;
    ...
    $mediauri = ReturnVideoUriHashed($VidInfo);
    
    $achannelvideoInfoArr['vid_id'] = $vid_id;
    $achannelvideoInfoArr['mediauri'] = $mediauri;
    $achannelvideoInfoArr['Title'] = $Title;
    $achannelvideoInfoArr['photoXSmallVideoInfo'] = photoReturnSrcXSmall($VidInfo);
    $achannelvideoInfoArr['duration'] = $duration;
    $achannelvideoInfoArr['views'] = $views;
    $achannelvideoInfoArr['PlaysCount'] = displayPlaysCount($VidInfo['nb_views'],0);
    $achannelvideoInfoArr['formatDate'] = formatDateAsString($pdate);
    $achannelvideoInfoArr['Description'] = $Description;
    $achannelvideoInfoArr['Videocan_comment'] = $VidInfo['can_comment'];
    $achannelvideoInfoArr['Videocan_share'] = $VidInfo['can_share'];
    $achannelvideoInfoArr['privacy_social'] = intval($channelPrivacyArray['privacy_social']);
    $achannelvideoInfoArr['views'] = $views;
    
    
    
    $channelvideoInfoArr[] = $achannelvideoInfoArr;
}
$data['channelvideoInfoArr'] = $channelvideoInfoArr;



$pagcount = floor($channelvideoInfoCount / $limit);
if (($channelvideoInfoCount % $limit) != 0) {
    $pagcount++;
}

$data['pagcount'] = $pagcount;
$data['currentpage'] = $currentpage;
$data['loadMoreLink'] = ReturnLink('channel-videos/' . $currentchannelname . '/page/' . ($currentpage + 1));
$data['pageArg'] = UriArgIsset('page');
$data['loadPreviousLink'] = ReturnLink('channel-videos/' . $currentchannelname . '/page/' . ($currentpage - 1));
