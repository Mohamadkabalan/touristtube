...
class DataMapper implements IteratorAggregate {
	...
	static $_dmz_config_defaults = array(
		...
		'field_label__format' => '${model}_${field}',
	...
	public function DataMapper($id = NULL)
	{
		...
		if ( ! isset(DataMapper::$common[$common_key]))
		{
			...
			if(!empty($this->lang_file_format))
			{
				...
				$deft_ = $this->config->item('language');
				$idiom = ($deft_ == '') ? 'english' : $deft_;
				if(file_exists(APPPATH.'language/'.$idiom.'/'.$lang_file.'_'.EXT))
				{
	...
	public function localize_label($field, $label = FALSE)
	{
		...
		{
			$label = $field;
			if(!empty($this->field_label__format))
			{
				$label = $this->localize_by_model($this->field_label__format, $field);
				if($label === FALSE)
				{
					$label = $field;
				}
