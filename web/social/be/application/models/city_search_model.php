<?php
class City_Search_Model extends CI_Model {
    public function search($term, $country){
        ...
        $ci_res = $this->db->select('webgeocities.id as id, webgeocities.name as city_name, states.state_name, cms_countries.name as country_name')
                ->from('webgeocities')
                ->join('states', 'webgeocities.state_code = states.state_code and webgeocities.country_code = states.country_code', 'left')
                ->join('cms_countries', 'cms_countries.code = webgeocities.country_code', 'inner')
                ->like('webgeocities.name', $term, 'after')
                ->like('webgeocities.country_code', $country)
                ->order_by('webgeocities.order_display', 'desc')
                ->order_by('webgeocities.name', 'asc')
                ->get()
                ->result();
