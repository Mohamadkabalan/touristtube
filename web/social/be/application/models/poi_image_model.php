...
class Poi_Image_Model extends CI_Model {
    ...
    public function get_file($file_id)
   {
    ...
    public function update_default_pic($poi_id,$image_id){
        $default_pic = array('default_pic'=> 0);
        $update_default_pic= array('default_pic'=> 1);
        $this->db->where('poi_id', $poi_id);
        $result = $this->db->update('discover_poi_images', $default_pic);
        if($result == TRUE){
            $this->db->where('poi_id', $poi_id);
            $this->db->where('id', $image_id);
            $result = $this->db->update('discover_poi_images', $update_default_pic);
            $result = TRUE;
        }
        return $result;
    }
