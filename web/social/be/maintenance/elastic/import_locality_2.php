...
$query = "SELECT *,name as autocomplete_destinct,name as namePh, 1 as type FROM `webgeocities`";
$rs    = mysqli_query($conn, $query);
$count = mysqli_num_rows($rs);
if ($count < 1) die('No records to import');
...
$query2         = "SELECT code as countryCode, name as countryName, name as countryNamePh, name as autocomplete_destinct, full_name as countryFullname, full_name as countryFullnamePh, ioc_code as countryThreeCode,iso3 countryISO3Code, popularity as popularity, search_hotels as search_hotels, 3 as type FROM `cms_countries`";
...
foreach ($countriesArray as $country) {
    ...
    $translation            = array();
    $translation['name_en'] = $country['countryName'];

    $queryC = "SELECT * FROM `ml_countries` WHERE `code` = '".$country['countryCode']."' ;";
    $rsC    = mysqli_query($conn, $queryC);

    if ($rsC) {
        while ($rowC = mysqli_fetch_assoc($rsC)) {
            $translation['name_'.$rowC['lang_code']] = $rowC['name'];
        }
    }
...
while ($row = mysqli_fetch_assoc($rs)) {
    $id                      = $row['type']."-".$row['id'];
    $translationW            = array();
    $translationW['name_en'] = $row['name'];
    $queryW                  = "SELECT * FROM `ml_webgeocities` WHERE `city_id` =".$row['id'].";";
    $rsW                     = mysqli_query($conn, $queryW);
    if ($rsW !== false) {
        while ($rowW = mysqli_fetch_assoc($rsW)) {
            $translationW['name_'.$rowW['lang_code']] = $rowW['name'];
        }
		
		$row['translation'] = $translationW;
		mysqli_free_result($rsW);
    }
