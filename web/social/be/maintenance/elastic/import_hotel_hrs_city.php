...
$index = "tt_hrshotelscities";
$type = "hrshotelscities"; 

$query = "SELECT *, count(location_id) location_count FROM
            (
                SELECT 
                		w.id, w.name as name, LCASE(w.name) as name_key, w.popularity, w.longitude, w.latitude,
                ...
                FROM webgeocities as w
                	INNER JOIN cms_hotel_city as chc ON chc.city_id = w.id
                	INNER JOIN cms_hotel_source as chs ON (chs.location_id = chc.location_id AND chs.is_active = 1)
                	INNER JOIN cms_hotel as h ON chs.hotel_id = h.id
                ...
                GROUP BY chc.city_id, chc.location_id
            )tbl
            GROUP BY id
...
if ($rs    = mysqli_query($conn, $query)) {
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
        ...
        if($row['location_count'] > 1) unset($row['location_id']);
