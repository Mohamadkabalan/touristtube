...
$index = "tt_deals";
$type  = "deals";
$query = "SELECT 'top_attractions' as type, dta.id, null as code, dta.name as name, dta.name as name_key, dta.description as description, dta.image_url as image, dta.deal_api_supplier_id, 100 as popularity,
          ...
          FROM deal_top_attractions as dta
          INNER JOIN webgeocities as w ON w.id = dta.city_id
...
if ($rs    = mysqli_query($conn, $query)) {
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
        ...
        $row['titleLocation'] = $row['name'].' '.$row['cityName'].(isset($row['countryName']) ? ' '.$row['countryName'] : '');
        $row['details'] = array("api_id"=>null,"type_id"=>null,"start"=>null,"end"=>null,"duration"=>null);
        $row['media'] = array("image"=>$row['image']);
        $row['vendor'] = array(
                "id"=>$row['deal_api_supplier_id'],
                "name"=>null,
                "description"=>null,
                "location"=>array(
                    "city"=> array("id"=>null,"code"=>null,"parent_code"=>null,"name"=>null),
        ...
        $row['price'] = array("currency_code"=> null,"amount_before_discount"=>null,"amount"=>null);
        $row['stars'] = null;
        $row['translation'] = array();
        $row['published'] = 1;
        $row['last_updated'] = null;
        
        unset($row['deal_api_supplier_id']);unset($row['image']);unset($row['cityId']);unset($row['cityName']);unset($row['cityPopularity']);
