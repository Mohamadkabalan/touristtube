...
function CreateCuisineEditor(text, id, is_new){
...
//    controller = GetController(window.location.pathname);
    $('.fedit').click(function(){
        ...
        var text = $(this).html();
        $(this).html(CreateCuisineEditor(text, id, 0));
    ...
    $('.container').on('click', '#lSave', function(e){
        ...
        var url = '';
        if(div_class === 'fedit'){
            url = './' + controller + '/ajax_cuisine_save/t/'+new Date().getTime();
        }else{
            url = './' + controller + '/ajax_ml_cuisine_save/t/'+new Date().getTime();
        }
        $('#imgSaving').show();
        $.ajax({
           url: url,
    ...
    $('.container').on('click', '.fadd', function(e){
        ...
        if(currentfedit && currentfedit !== id)
            $('#lCancel').trigger('click');
        $parent.html(CreateCuisineEditor('', id, 1));
