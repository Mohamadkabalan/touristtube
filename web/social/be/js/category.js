var controller = 'poi';
function CreateEditor(text, id, is_new){
...
//    controller = GetController(window.location.pathname);
    $('.fedit').click(function(){
        ...
        var text = $(this).html();
        $(this).html(CreateEditor(text, id, 0));
    ...
    $('.container').on('click', '#lSave', function(e){
        ...
        var url = '';
        if(div_class === 'fedit'){
            url = './' + controller + '/ajax_category_save/t/'+new Date().getTime();
        }else{
            url = './' + controller + '/ajax_ml_poi_categories_save/t/'+new Date().getTime();
        }
        $('#imgSaving').show();
        $.ajax({
           url: url,
           ...
           success: function (res){
               ...
               if(res.success){
                   ...
                   $('#lCancel').trigger('click');
                   alert("An error occured while trying to edit this category.")
               }
           },
