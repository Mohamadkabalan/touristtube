...
$new_sel= explode('_',$selected);
 ...
 $Query="update tt.discover_poi_latest set city_id=$nw_id,status=56 where id=$nd_id";
...
$new_sel= explode('_',$selected);
 ...
 $Query="update tt.discover_poi_latest set status=9 where id=$nd_id";
 if (mysqli_query($conn, $Query ))
		    {
...
$sql="SELECT  d.id AS d_id, d.name AS d_poi, d.state_code AS d_state, d.city AS d_city,d.state as d_statename, w.name AS w_city, d.latitude AS d_lat, w.latitude AS w_lat, d.longitude AS d_long, w.longitude AS w_long, d.city_id AS d_cityid, w.id AS w_id, w.state_code
FROM `discover_poi_latest` AS d
JOIN webgeocities AS w ON w.name like concat('%',d.city,'%')
AND d.country = w.country_code
AND d.country = 'JP'
AND d.state_code=w.state_code
AND d.published =0 
AND d.status=14
ORDER BY d.name DESC limit 500 "; 
...
while($row = mysqli_fetch_array($result1))
{
 ...
 print "<tr><td>$d_id</td><td>$d_poi </td><td> $d_city($d_statename)</td><td>$w_city($statecode)</td><td><input type= checkbox name=checked[$i] value=$i><textarea name=check_value$i id='check' type=hidden style='display:none;'>$check_value</textarea>
...
function addRowHandlers(tableId) {
	if(document.getElementById(tableId)!=null){
        ...
        lat+","+lng+"&key=AIzaSyAmiruPgOma0q3A5grcnwqfEuXyM0z_h4Q";
