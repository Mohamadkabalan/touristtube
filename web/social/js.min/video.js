    ...
    _V_.Component = _V_.Class.extend({
        ...
        fadeIn: function() {
            /*this.removeClass("vjs-fade-out");
             ...
             $("#PlayerButtons").removeClass("vjs-fade-out");
             $("#PlayerButtons").addClass("vjs-fade-in");*/
        },
        fadeOut: function() {
            /*this.removeClass("vjs-fade-in");
             this.addClass("vjs-fade-out");
             $("#PlayerButtons").removeClass("vjs-fade-in");
             $("#PlayerButtons").addClass("vjs-fade-out");*/
        },
...
// Seek Bar and holder for the progress bars
    _V_.SeekBar = _V_.Slider.extend({
        ...
        init: function(player, options) {
            this._super(player, options);
            this.addEvent("mousemove", this.onMouseMoveSep);
            this.addEvent("mouseout", this.onMouseOut);
        },
        createElement: function() {
            return this._super("div", {
                className: "vjs-progress-holder",
                innerHTML: '<div class="vjs-thumb-popup" id="vjs-thumb-popup"></div>'
            });
        },
        getPercent: function() {
        ...
        onMouseMoveSep: function(event) {
            var disTime = this.calculateDistance(event);
            var playerDuration = this.player.duration();
            var newTime = disTime * playerDuration;

            // Don't let video end while scrubbing.
            if (newTime == this.player.duration()) {
                newTime = newTime - 0.1;
            }
            var lastTime = parseInt(newTime);

            this.ApplyImage(lastTime);
            this.MoveImage(disTime);

        },
        ApplyImage: function(lastTime) {
            if (lastTime != 0) {
                var imageURL = timeFuncHtml(lastTime);
                document.getElementById("vjs-thumb-popup").style.display = 'block';
                document.getElementById("vjs-thumb-popup").innerHTML = '<img src="' + imageURL + '" width="76" height="46" />';
            } else {
                document.getElementById("vjs-thumb-popup").style.display = 'none';
            }
        },
        MoveImage: function(disTime) {
            var percentMove = 0;
            if (disTime >= 0.93) {
                percentMove = '88%';
            } else if (disTime <= 0.056) {
                percentMove = '0%';
            } else {
                percentMove = ((disTime - 0.055) * 100) + '%';
            }
            document.getElementById("vjs-thumb-popup").style.left = percentMove;
        },
        onMouseOut: function(newTime) {
            document.getElementById("vjs-thumb-popup").style.display = 'none';
        },
        onMouseUp: function(event) {
            this._super(event);
    ...
    _V_.LoadProgressBar = _V_.Component.extend({
        ...
        createElement: function() {
            return this._super("div", {
                ...
                innerHTML: '<span class="vjs-control-text">'+t("Loaded")+': 0%</span>'
            });
        },
        update: function() {
            if (this.el.style) {
    ...
    _V_.PlayProgressBar = _V_.Component.extend({
        createElement: function() {
            return this._super("div", {
                ...
                innerHTML: '<span class="vjs-control-text">'+t("Progress")+': 0%</span>'
            });
        }
    ...
    _V_.MuteToggle = _V_.Button.extend({
        ...
        createElement: function() {
            return this._super("div", {
                ...
                innerHTML: '<div><span class="vjs-control-text">'+t("Mute")+'</span></div>'
            });
        },
