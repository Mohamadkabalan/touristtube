...
/*function InitUploader() {
    ...
    function attachCallbacks(uploader) {
        ...
        uploader.bind('UploadFile', function (up, file) {
            ...
            upload_started = 1;
            SetPageLeave();
            $('#realupload_' + fid + ' .pause').show();
            $('#log li#realupload_' + fid + ' span.status').html(t("Uploading ..."));
            $('#log li#realupload_' + fid).find('span.progressvalue').text('0%');
        });

        uploader.bind('UploadProgress', function (up, file) {

            var fid = file.id;
            $('#log li#realupload_' + fid).find('div.progress').css('width', file.percent + '%');
            $('#log li#realupload_' + fid).find('div.progress').css('background-color', '#f0bf1b');
            $('#log li#realupload_' + fid).find('span.progressvalue').text(file.percent + '%');
            //console.log(up);
        });
        ...
        uploader.bind('FileUploaded', function (up, file, response) {
            ...
            var fid = file.id;
            var item = $('#log li#realupload_' + fid);
            ...
            if (Jresponse.status != 'ok') {
                ...
                item.addClass('success').find('span.status').html('<span class="status2">' + t("Upload Complete") + '</span>' + Jresponse.msg);

                FileUploadCompleted(fid);
                showHideSameDescription();
                //set thumbnail
                var thumb = $('#realupload_' + fid + ' input[name=vthumb]').val();
                $('#realupload_' + fid + ' .stanleft .stanleftin').html('<img src="' + AbsolutePath + "/" + thumb + '" width="86" height="48">');
                $('#realupload_' + fid + ' .SaveButton').removeAttr('disabled');

                $('#realupload_' + fid + ' .stanleft .stanleftin').css('background', 'none');
                $('#realupload_' + fid + ' .togb').show();
                if (countfileupload == countfileuploadnumber) {
                    if (!$('.ax-file-list li').find('.togb').hasClass('closeclose')) {
                        $('.ax-file-list li').first().find('.togb').click();
                    }
...
$(document).ready(function(){
    ...
    $('#real_upload').ajaxupload({
        ...
        url: ReturnLink('/upload_real2.php'),               //php/asp/jsp upload script
