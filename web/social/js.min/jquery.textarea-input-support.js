...
(function($) {
		...
		_initSource: function() {
			...
			if ( $.isArray(this.options.source) ) {
				...
				this.source = function( request, response ) {
					...
					//ADDED: added so that listed items dont show in auto complete ADDED_01
					var currentVals = $(this.element).val().split(' ');
					var array = this.options.source.clone();
					for(var i = 0; i < array.length ; i++ ){
						if( currentVals.elementExists( array[i] ) ){
							array.remove(i);
							i--;
						}
					}
		...
		menuOpt: function(self, doc){
			return {
				...
				select: function( event, ui ) {
					...
					//execute select callback if exists
					if( self.options.onselect ) self.options.onselect(self, item);
	...
	$.extend( $.ui.textareainputsupport.prototype.options, {
		select: function( event, ui ) {
			...
			//in case no item is selcted but enter was pressed (for submit)
			if( !ui.item ) return false;
			
			//ADDED: Dont add something that already exists ADDED_02
			//ADDED: this is needed in case the remove exisiting ADDED_01 is used above
			if( $(this).val().indexOf( ui.item.value ) == -1 ){
				$(this).replaceSelectedText(ui.item.value + " ");
			}else{
				var selected = $(this).val().substr(curPos.start, curPos.end - curPos.start );
				//dont delete
				if( selected != ui.item.value )
