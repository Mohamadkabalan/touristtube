...
function verifyForm(which) {
    ...
    $('input,select,textarea', $currobjselected).removeClass('err').each(function () {
        ...
        if ((name == 'location') || (name == 'location_name') || (name == 'cityname') || (name == 'status') || (name == 'vid') || $(this).hasClass('filevalue')) {
            ...
            if ((!$('.uploadinfomandatory' + name, $parenttarget).hasClass('inactive') && $('.uploadinfomandatory' + name, $parenttarget).css('display') != 'none' && $(this).val() == '') || (name == "category" && !$('.uploadinfomandatorycategory', $parenttarget).hasClass('inactive') && $(this).val() == '0') || (name == "country" && !$('.uploadinfomandatorycountry', $parenttarget).hasClass('inactive') && $(this).val() == '0')) {
...
function addAutoComplete(which) {
    ...
    $citynameaccent.autocomplete({
        ...
        select: function (event, ui) {
            ...
            //$citynameaccent.val(ui.item.label);
        ...
        change: function (event, ui) {
            if (!ui.item) {
                ...
                //dont show the city accent. only show the regular city.
                //$citynameaccent.val( getCity.label );
...
function createImage(file, i) {
    ...
    var template = '<li id="SWFUpload_2_' + i + '" data-id="' + i + '" data-isImage="' + isImage + '">' +
            ...
            '<div class="uploadinfocheckbox_upload uploadinfocheckbox3" style="display:none;"><div class="uploadinfocheckboxpic"></div></div>' +
            '</div>' +
            ...
            TTAlert({
                ...
                btn2Callback: function (data) {
                    if (data) {
                        ...
                        // Reduce the counter and hide its display.
                        if (countfileupload > 1)
                            countfileupload--;
                        if (countfileuploadnumber > 1)
                            countfileuploadnumber--;
                        $('.upload-loading-txt-file').hide();
                        $('.stop_all_upload').hide();
                        $('.upload-loading-sep').hide();
...
function addButtonSave(myid, isimage) {
    var id = "#" + myid;
    $(document).on("click", "" + id + " .SaveButton", function () {
...
$(document).ready(function () {
    ...
    $(document).on('click', ".upload2 .togb", function () {
        ...
        if (this_button.parent().find(".uploadinfocheckboxAllcontainer .uploadinfocheckbox3").hasClass("active") && this_button.parent().find(".uploadinfocheckboxAllcontainer .uploadinfocheckbox3").css("display") != "none")
            return 0;

        if (this_button.hasClass("openclose")) {
            var objOpenBool = false;
            if (currentsection == "upload-album") {
                if ($('.uploadalbumselected .uploadinfocheckbox_apply4,.uploadalbumselected .uploadinfocheckbox4').hasClass('active')) {
                    $('.uploadalbumselected .uploadinfocheckbox_apply4,.uploadalbumselected .uploadinfocheckbox4').click();
                }
            }
            $(".upload2 #log li").each(function () {
                var thisObject = $(this);
                ...
                TTAlert({
                    ...
                    btn2Callback: function (data) {
                        if (data) {
                            ...
                            setTimeout(function () {
                                that.find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').removeClass('active');
    ...
    if (currentsection == "upload-album") {
        ...
        $(document).on('click', '.' + currentlogalbumaddid + ' .UploadMediaRightTF .ClearNewAlbumButton', function () {
            ...
            var $currobjselected = $('.' + currentlogalbumaddid);
            $('.uploadinfomandatory span', $currobjselected).html('');
        ...
        $(document).on('click', '.' + currentlogalbumaddid + ' .UploadMediaRightTF .SaveNewAlbumButton', function () {
            if (verifyForm(currentlogalbumaddid)) {
                ...
                $.post(ReturnLink('/ajax/ajax_addnewalbumupload.php'), {title: getObjectData($("." + currentlogalbumaddid + " input[name=title]")), globchannelid: globchannelid, cityname: $("." + currentlogalbumaddid + " input[name=cityname]").val() , cityid: $("." + currentlogalbumaddid + " input[name=cityname]").attr('data-id'), citynameaccent: $("." + currentlogalbumaddid + " input[name=citynameaccent]").val(), is_public: USER_PRIVACY_PUBLIC, description: getObjectData($("." + currentlogalbumaddid + " textarea[name=description]")), category: $("." + currentlogalbumaddid + " select[name=category] option:selected").val(), placetakenat: getObjectData($("." + currentlogalbumaddid + " input[name=placetakenat]")), keywords: getObjectData($("." + currentlogalbumaddid + " input[name=keywords]")), country: $("." + currentlogalbumaddid + " select[name=country] option:selected").val(), location: $("." + currentlogalbumaddid + " input[name=location]").val()}, function (data) {
                    if (data != false) {
            ...
            if (verifyForm('logalbumelected')) {
                ...
                $.post(ReturnLink('/ajax/ajax_updatealbumupload.php'), {catalog_id: CatalogID(), title: getObjectData($(".logalbumelected input[name=title]")), cityname: $(".logalbumelected input[name=cityname]").val() , cityid: $(".logalbumelected input[name=cityname]").attr('data-id'), citynameaccent: $(".logalbumelected input[name=citynameaccent]").val(), is_public: USER_PRIVACY_PUBLIC, description: getObjectData($(".logalbumelected textarea[name=description]")), category: $(".logalbumelected select[name=category] option:selected").val(), placetakenat: getObjectData($(".logalbumelected input[name=placetakenat]")), keywords: getObjectData($(".logalbumelected input[name=keywords]")), country: $(".logalbumelected select[name=country] option:selected").val(), location: $(".logalbumelected input[name=location]").val()}, function (data) {
                    ...
                    if (data != false) {
                        ...
                        if ($('.uploadalbumselected .uploadinfocheckbox4').hasClass('active')) {
                            ...
                            $('.uploadalbumselected .uploadinfocheckbox_upload').removeClass('active');
                                    },
                                    error: function (jqXHR, textStatus, errorThrown) {
                            $('.uploadalbumselected .uploadinfocheckbox_upload').removeClass('active');
        ...
        $(document).on('click', ".uploadalbumselected .togb", function () {
            ...
            if (this_button.hasClass("openclose")) {
                ...
                that.animate({height: "387px"}, 500, function () {
                    this_button.removeClass("openclose").addClass("closeclose");
                    $(window).resize();
                });
            } else {
    ...
    $(document).on('click', ".uploadinfocheckbox_upload", function () {
        if ($(this).hasClass('active')) {
            ...
            if ($(this).hasClass('uploadinfocheckbox4')) {
                $('.upload2 #log li').each(function () {
                    $(this).find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').hide();
                    $(this).find('.licorner .togb').show();
                    $(this).find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').removeClass('active');
                });
            }else if ($(this).hasClass('uploadinfocheckbox3')) {
                ...
                $('.upload2 #log li').each(function () {
                    if ($(this).find('.togb').css('display') != 'none') {
                        if ($(this).attr('id') != currid) {
                            ...
                            targetlater.click();
                            $(this).find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').addClass('active');
                            $(this).find('.uploadinfocheckboxcontainer .uploadinfocheckbox4').removeClass('active');
                            $(this).find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').show();
...
function doalbumdatadisplay(obj) {
    ...
    if (id != 0) {
        ...
        $.post(ReturnLink('/ajax/ajax_getalbumuploaddata_channel.php'), {id: id}, function (data) {
            if (data != false) {
...
function CopyForm(obj) {
    ...
    var formarrstr = "";
    if (currentsection == "upload-album") {
        obj.find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').addClass('active');
...
function CopyFormtab2(obj) {
    ...
    if (currentsection == "upload-album") {
        obj.find('.licorner .uploadinfocheckboxAllcontainer .uploadinfocheckbox3').addClass('active');
        if (obj.find(".uploadinfocheckbox4").hasClass("active")) {
...
function formIsFilled(obj) {
    var ok = false;
    if (getObjectData($('input[name=title]', obj)) != "") {
        ok = true;
    } else if (getObjectData($('input[name=citynameaccent]', obj)) != "") {
        ok = true;
    } else if (getObjectData($('textarea[name=description]', obj)) != "") {
        ok = true;
    } else if (getObjectData($('input[name=placetakenat]', obj)) != "") {
        ok = true;
    } else if (getObjectData($('input[name=keywords]', obj)) != "") {
        ok = true;
...
function showHideSameDescription(){
    if (currentsection == "upload-album") {
        if($('#log li.success').length>0){
            $('.uploadinfocheckbox4').show();
        }else{
            $('.uploadinfocheckbox4').hide();
        }
    }else{
