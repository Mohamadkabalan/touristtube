...
function PlayMedia(){
	...
	//estimate speed
	var imageAddr = AbsolutePath + "/images/speedtest.png?n=" + Math.random();
...
function EmbedFlashJW(where, res_list, regWidth, regHeight, ThumbPath , videorelated_id,show_related,is_post,auto_start){
    ...
    var mobj;
    //mobj = {"file":"/"+res_listarr[0] , "label":"Auto","default": "true"};
    //res_real.push(mobj);
    for(var i=0; i<res_listarr.length; i++){
        mobj = {"file":"/"+res_listarr[i] , "label":res_names[i]};
        res_real.push(mobj);
    }
    if(videorelated_id==0){
        jwplayer(where).setup({
            ...
            logo: {
                file: ReturnLink('/images/Logo.png'),
        ...
        jwplayer(where).setup({ 
            ...
            autostart: auto_start,                
            logo: {
                file: ReturnLink('/images/logovideo.png'),
                link: 'https://www.touristtube.com/'
            },
            skin: {
            name: "glow"
            ...
            related: {
                file: ReturnLink("ajax/getVideoRelatedXML.php?id="+videorelated_id),
                onclick: "link"
            },
            tracks: [{ 
		...
		callback: function(data){
			...
			if( data.status == 'ok' ){
				...
				$('#rating img.rating_star').each(function(i){
					if(i < ratingi){
						$(this).attr('src', AbsolutePath + '/images/rating_1.png');
					}else{
						$(this).attr('src', AbsolutePath + '/images/rating_0.png');
					}
				});
			}
...
$(document).ready(function(){
	$('#endOfPlayReplay').click(function(){
		$('#endofplay').hide();
		//replay
	});
	var opened = false;
	$("#stanRes").live('click',function(){
		if(opened){
			$("#listRes").stop().animate({height:'0px'},500,function(){});
			opened = false;
		}else{
	...
	$(".ratingcontainer li").live('click',function(){
	...
	$(".likeUnlike").live('click',function(){
