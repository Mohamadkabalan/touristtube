...
class PS_Pagination {
	...
	function paginate() {
		...
		$all_rs = @db_query($this->sql );
		if (! $all_rs) {
			if ($this->debug)
				echo "SQL query failed. Check your query.<br /><br />Error Returned: " . db_error();
			return false;
		}
		$this->total_rows = db_num_rows($all_rs );
		@db_close($all_rs );
		...
		$rs = @db_query($this->sql . " LIMIT {$this->offset}, {$this->rows_per_page}" );
		if (! $rs) {
			if ($this->debug)
				echo "Pagination query failed. Check your query.<br /><br />Error Returned: " . db_error();
			return false;
		}
		return $rs;
	}
