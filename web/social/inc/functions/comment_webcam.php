<?php
/**
 * comment on webcams functions
 * @todo Move to <b>webcams.php</b>
 * @see webcams
 * @package comment_webcam
 */

//THIS FILE IS A COPY OF comment.php DUPLICATE ALL CHANGES.

/**
 * Add a comment to a video
 * @param integer $user_id the person making the comment
 * @param integer $video_id the webcam being commented on
 * @param string $text the comment text
 */
function commentAdd($user_id, $video_id, $text) {
	$comment_id = socialCommentAdd($user_id, $video_id, SOCIAL_ENTITY_WEBCAM , $text, 0);
	if ($comment_id){
