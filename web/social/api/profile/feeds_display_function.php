...
function feeds_display($news_feed){
    ...
    foreach ( $news_feed as $vfeeditem ){    //echo "<pre>";print_r($vfeeditem['media_row']);die;    //echo $vfeeditem['entity_type'].'_'.$vfeeditem['action_row']['entity_type'];die; 
        if($vfeeditem['action_type'] == SOCIAL_ACTION_LIKE && ($vfeeditem['entity_type'] == SOCIAL_ENTITY_RESTAURANT_REVIEWS || $vfeeditem['entity_type'] == SOCIAL_ENTITY_LANDMARK_REVIEWS ||$vfeeditem['entity_type'] == SOCIAL_ENTITY_HOTEL_REVIEWS ||$vfeeditem['entity_type'] == SOCIAL_ENTITY_AIRPORT_REVIEWS) && $vfeeditem['action_row']['entity_type'] == SOCIAL_ENTITY_COMMENT && (!isset($videoRecord['published']) || intval($videoRecord['published']) == -2)){
            continue;
        }
     ...
     //echo "<pre>";print_r($post_video_link);die;  
        /**/
        if (isset($videoRecord['channelid']) && intval($videoRecord['channelid']) > 0) {
            $channel_id = intval($videoRecord['channelid']);
        }else if (isset($videoRecord['channel_id']) && intval($videoRecord['channel_id']) > 0) {
        ...
        $video_photo = (isset($videoRecord['type']) && strstr($videoRecord['type'],'video') == null ) ? 'photo' : 'video';
        ...
        switch($vfeeditem['action_type']){
                case SOCIAL_ACTION_UPLOAD:
									...
									foreach ($action_row_other as $item){
										...
										if($item['media_row']['id'] != $vfeeditem['entity_id'] ){
											if ($item['media_row']['image_video'] == "v") {
												$thumb_src_link = substr(getVideoThumbnail($item['media_row']['id'], $path.$item['media_row']['fullpath'], 0), 0);
												if ($thumb_src_link)
												{
													$thumb_src = explode($path, $thumb_src_link);
													if (count($thumb_src) == 2)
						...
						if ($entity_type == SOCIAL_ENTITY_POST)
							$title = $videoRecord['post_text'];
                        else if (isset($videoRecord['title']))
							$title = $videoRecord['title'];
						
                        $description = (isset($videoRecord['description']))?$videoRecord['description']:'';
                        $description = htmlEntityDecode($description, 2);
                        //$target_name = $title;
                        ...
                        //$fullLink = photoReturnSrcSmall($videoRecord);
                         if ( isset($videoRecord['image_video']) && $videoRecord['image_video'] == "v" ) {
                            $thumb_src_link = substr(getVideoThumbnailByCode($videoRecord['code'], $path.$videoRecord['fullpath'], 0), 0);
                            if ($thumb_src_link)
							{
								$thumb_src = explode($path, $thumb_src_link);
								if (count($thumb_src) == 2)
                            ...
                            $thumb_src = (isset($videoRecord['fullpath']))?$videoRecord['fullpath'] . $videoRecord['name']:'';
                            $type = 'image';
                        }
                        ...
                        switch($entity_type){
                                case SOCIAL_ENTITY_USER_EVENTS:
                                        ...
                                        $description = htmlEntityDecode($videoRecord['description'], 2);
                             ...
                             */
                                        ...
                                        if($videoRecord['media_file']){
                                            if ($videoRecord['is_video'] == 0) {
                                               // $repath = relativevideoReturnPostPath($videoRecord);
                                                ...
                                                $picthumb_img = getVideoThumbnail_Posts($videoCode, $path.$repath, 0);
												if ($picthumb_img)
												{
													$picthumb_img = explode($path, $picthumb_img);
													if (count($picthumb_img) == 2)
                                ...
                                case SOCIAL_ENTITY_FLASH:
                                    ...
                                    $echo_text = $videoRecord['flash_text'];
...
//                                            );
                                            ...
                                            if ($photo['image_video'] == "v") {
                                                $thumb_src_link = substr(getVideoThumbnail($photo['id'], $path . $photo['fullpath'], 0), 0);
                                                if ($thumb_src_link)
												{
													$thumb_src = explode($path, $thumb_src_link);
													if (count($thumb_src) == 2)
...
//                                        $channel_array=channelGetInfo($channelDetailArray['channelid']);
                                ...
                                case SOCIAL_ENTITY_EVENTS_LOCATION:
                                    if($owner_type == 'channel'){
                                        ...
                                        $target_name = htmlEntityDecode($videoRecord['name'], 2);
                                        $target_id = $videoRecord['id'];
                                        break;
                ...
                case SOCIAL_ACTION_CONNECT:
                        ...
                        if($entity_type == SOCIAL_ENTITY_CHANNEL){
                            ...
                            $description=htmlEntityDecode($videoRecord['small_description'], 2);
                        }
                        if($action_row_count > 1){
                ...
                case SOCIAL_ACTION_SPONSOR:
                    ...
                    switch($entity_type){
                        case SOCIAL_ENTITY_CHANNEL:
                            ...
                            $description = htmlEntityDecode($videoRecord['small_description'], 2);
                            $title = $target_name;
                            $overhead_text = htmlEntityDecode($actionRecord['msg'], 2);
                            break;
                        case SOCIAL_ENTITY_EVENTS:
//                           $action = "$staticaction an event %s";
                    ...
                    if($action_row_count > 1){
                        ...
                        $uslnkothname= htmlEntityDecode($channel_arraygroup['channel_name'], 2);       
                        $other = $uslnkothname;
                        $otherId = $action_row_other['user_id'];
                    }  
                break;
                ...
                case SOCIAL_ACTION_RELATION_SUB:
                    ...
                    $channel_name_r = htmlEntityDecode($videoRecord['channel_name'], 2); /*parent channel name*/
                ...
                case SOCIAL_ACTION_LIKE:
                        ...
                        $from_id = (isset($videoRecord['userid']))?intval($videoRecord['userid']):0;
...
//                                        $target_name = $title;
//                                        $target_id = $videoRecord['id'];
                                        $overhead_text = htmlEntityDecode($vfeeditem['original_media_row']['comment_text'],2);
                                    }else{      
                                            if ($action_row_count){
...
//                                        break;
    ...
    //                                        }
                                            ...
                                            if ($media_row['image_video'] == "v") {
                                                $thumb_src_link = substr(getVideoThumbnail($media_row['id'], $path . $media_row['fullpath'], 0), 0);
                                                if ($thumb_src_link)
												{
													$thumb_src = explode($path, $thumb_src_link);
													if (count($thumb_src) == 2)
...
//                                            );
                ...
                case SOCIAL_ACTION_COMMENT:
                        $overhead_text = htmlEntityDecode($actionRecord['comment_text'],2);                        
                        $staticaction = "[ActionOwner] commented on";                        
                        if($action_row_count > 1){
...
//                                            );
                                ...
                                case SOCIAL_ENTITY_HOTEL_REVIEWS:  //$target_name='mukesh4';break;
                                        $item_data = getHotelInfo($videoRecord['hotel_id']);
                                ...
                                break;    
                                case SOCIAL_ENTITY_LANDMARK_REVIEWS:
                                        $item_data = getPoiInfo($videoRecord['poi_id']);
                                ...
                                break;
                                case SOCIAL_ENTITY_AIRPORT_REVIEWS:
                                        $item_data = getAirportInfo($videoRecord['airport_id']);
                                        $target_name = $item_data['name'];
                                        $target_id = $item_data['id'];
                                    if ($action_row_count){
                                        $action="$staticaction the [review]";
...
//                                $loop_ret_row['user_profile_pic']= 'media/tubers/' . $pic;
                                ...
                                if ($action_row_count){$action_text .= '';
                                ...
                                $target_name = htmlEntityDecode($videoRecord['channel_name'], 2);
                                $target_id = $videoRecord['id'];
                        }
						
                        $overhead_text = htmlEntityDecode($action_row['msg'], 2);
						
                        if(isset($videoRecord['user_id']) && $videoRecord['user_id'] == $user_id ) {
                                $channel_array ='';
                                if( $entity_type == SOCIAL_ENTITY_USER_EVENTS ) {
...
//                                $target_name = htmlEntityDecode($videoRecord['channel_name']);
//                                $target_id = $videoRecord['id'];
                        
                        }
						else if(isset($videoRecord['user_id']) && $action_row['from_user'] != $videoRecord['user_id'] ) { //echo $action_row['from_user'].'_'.$user_id;die;
                                $channel_array ='';
                                $owner_id=$user_id;
                                $userInfo_action_who = getUserInfo($owner_id);
                                if( $entity_type == SOCIAL_ENTITY_USER_EVENTS ) {
                                        ...
                                        $channel_array_action = channelGetInfo($videoRecord['channelid']);
                                        $channel_name = htmlEntityDecode($channel_array_action['channel_name'], 2);
                                        ...
                                        $channel_name = htmlEntityDecode($videoRecord['channel_name'], 2);
                                        if(strlen($channel_name) > 32){
                                           $channel_name = substr($channel_name,0,32).' ...';
                                        }
...
//        }
        ...
        $title = htmlEntityDecode($title, 2);
        
        //$username = returnUserDisplayName($action_profile);
        $pic = $action_profile['profile_Pic'];
        $gender = $action_profile['gender'];

        $overhead_text = htmlEntityDecode($overhead_text, 2);
        if(!isset($owner_id)){
        ...
        //Elie
        $loop_ret_row['target_name'] = htmlEntityDecode(trim($target_name), 2);
...
//            $loop_ret_row['user_profile_pic'] = ($channelInfo['logo']) ? photoReturnchannelLogo($channelInfo) : ReturnLink('/media/tubers/tuber.jpg');
        ...
        } else if ( ( isset($media_row['channelid']) && intval($media_row['channelid']) > 0) || (isset($media_row['channel_id']) && intval($media_row['channel_id']) > 0) ) {
            if (isset($media_row['channel_id']) && intval($media_row['channel_id']) > 0) {
...
//            $ischannel = 0;
        ...
        switch($entity_type){
            case SOCIAL_ENTITY_MEDIA:
                $thumbLink = $videoRecord['fullpath'] . $videoRecord['name'];
            ...
            case SOCIAL_ENTITY_CHANNEL:
                ...
                $loop_ret_row['share_image'] = $uricurserver.'/media/channel/' . $videoRecord['id'] . '/thumb/' . $videoRecord['header'];
                break;
        }
