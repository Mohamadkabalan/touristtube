 ...
 * 
 * @param S session id
 * @param id userid-> id of owner of the event
 * @param status event status (either 0,1,2,or3) Status: 0->Past events, 1->Upcoming, 2->Current, 3->current+upcoming
...
//		$status = intval( $_REQUEST['status'] );
	...
	$id = intval( $submit_post_get['user_id'] );
...
//		$to = $_REQUEST['to'];	
	...
	$usereventsInfo = userEventSearch($options);
	
    //echo "<pre>";print_r($usereventsInfo);   
	$output = array();
	...
	if($usereventsInfo){
		...
		foreach($usereventsInfo as $event){
			$thumbnail = ($event['photo']) ? getEventThumbPath($event,"") : ReturnLink('media/images/eventsdetailed/eventthemephoto.jpg');
                        ...
                        if($user_id){
                            ...
                            if(socialLiked($user_id, $event['id'], SOCIAL_ENTITY_USER_EVENTS))
                                $is_liked = 1;
                            else
                                $is_liked = 0;
                        }
