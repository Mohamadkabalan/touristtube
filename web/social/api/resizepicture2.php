...
//$keep_ratio = isset($_GET['keep_ratio']) ? ($_GET['keep_ratio'] == 1) : false;
$keep_ratio = $request->query->get('keep_ratio',1);
...
if (!file_exists($path . $thumbcache . $filename ) || $nocache_get) {
    ...
    if ( mediaSubsample($options) ) {
        $theFileLink = $path . $thumbcache . $filename;
    } else {
        $theFileLink = '';
    }
}