...
$new_sel= explode('_',$selected);
 ...
 $Query="update tt.prague_poi set city_id=$nw_id,status=58 where id=$nd_id";
...
$new_sel= explode('_',$selected);
 ...
 $Query="update tt.prague_poi set status=90 where id=$nd_id";
 if (mysqli_query($conn, $Query ))
      {
...
$sql="SELECT d.id AS d_id, d.name AS d_poi, d.website as web, d.state_code AS d_state, d.city AS d_city, d.n_city_local as n_city, w.name AS w_city, d.state AS d_statename, d.latitude AS d_lat, w.latitude AS w_lat, d.longitude AS d_long, w.longitude AS w_long, d.city_id AS d_cityid, w.id AS w_id, w.state_code
FROM `prague_poi` AS d
...
while($row = mysqli_fetch_array($result1))   
{
 ...
 print "<tr><td>$d_id</td><td>$d_poi </td><td> $d_city( $statecode)</td><td>$w_city($statecode)</td><td><input type= checkbox name=checked[$i] value=$i><textarea name=check_value$i id='check' type=hidden style='display:none;'>$check_value</textarea>
